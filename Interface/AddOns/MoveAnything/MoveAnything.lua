...
function MovAny:HideFrame(f, readOnly)
	...
	elseif fn == "Boss1TargetFrame" or fn == "Boss2TargetFrame" or fn == "Boss3TargetFrame" or fn == "Boss4TargetFrame" then
		f:UnregisterAllEvents()
		f:Hide()
		f.oldShow = f.Show
		f.Show = function()
...
function MovAny:UnanchorRelatives(e, f, opt)
	...
	if p.GetChildren and p:GetNumChildren() <= DBG_MAX_CHILDREN then
		...
		if children ~= nil then
			for i, v in ipairs(children) do
				if not self:IsProtected(v) and v.GetPoint then
					self:_AddDependents(relatives, v)
				end
			end
		end
	...
	if size > 0 then
		...
		for i, v in pairs(relatives) do
			if v:GetName() ~= nil and not self:IsContainer(v:GetName()) and not string.match(v:GetName(), "BagFrame[1-9][0-9]*") and not self.NoUnanchoring[v:GetName()] and not v.MAPoint then
				-- alternatively use not self:GetFrameOptions(v:GetName()) instead of v.MAPoint
	...
	for index, frameName in ipairs(ContainerFrame1.bags) do
		...
		if not bag or (bag and not MovAny:IsModified(bag, "pos") and not MovAny:GetMoverByFrame(bag)) then
			...
			elseif freeScreenHeight < frame:GetHeight() then
				...
				frame:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", select(4, lastFrame:GetPoint(1)), lastFrame:GetTop() + CONTAINER_SPACING)
			end
...
function MovAny_OptionsOnShow()
	...
	if type(MADB.characters) == "table" then
		...
		UIDropDownMenu_Initialize(MAOptProfileFrame, MovAny.ProfileDropDownInit)
		UIDropDownMenu_SetSelectedValue(MAOptProfileFrame, profile)
...
function MovAny:CreateVM(name)
	...
	vm.MAScanForChildren = function(vm, dontCallNewChild, dontSync)
		--[[
		xpcall(function() MAScanForChildren(vm, dontCallNewChild, dontSync) end, function()
