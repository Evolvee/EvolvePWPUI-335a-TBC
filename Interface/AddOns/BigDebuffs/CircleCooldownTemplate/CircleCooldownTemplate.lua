...
--######################################################################
--[[
Change path to your addon in xml file:
"Interface\AddOns\LoseControl\CircleCooldownTemplate\Cooldown\CircleCooldown"
"Interface\AddOns\LoseControl\CircleCooldownTemplate\Cooldown\edge"
]]
--[[------------------------ ADD API -----------------------------------
...
self:SetReverse(true/false)
self:SetDrawEdge(true/false)
self:SetDrawBling(true/false)
self:SetDrawSwipe(true/false)
...
local function SetFrameVisible(frame, visible)
	if visible then
		frame:Show()
        frame:SetFrameLevel(2) -- фикс для близз фреймов
	else
		frame:Hide()
	end
end
