local major = "LibHealComm-4.0" -- Backported by: Tsoukie
local minor = 108
...
local isTBC = WOW_PROJECT_ID_RCE == WOW_PROJECT_BURNING_CRUSADE_CLASSIC
local isWrath = WOW_PROJECT_ID_RCE == WOW_PROJECT_WRATH_CLASSIC
...
if( not HealComm.rankNumbers ) then
	HealComm.rankNumbers = setmetatable({}, {
		__index = function(tbl, index)
			local number = tonumber(strmatch(index, "(%d+)")) or 1
		...
		__index = function(tbl, index)
			...
			for id=1, (offset + numSpells) do
				...
				if( index == name ) then
					...
					local _, _, spellID = HealComm.tooltip:GetSpell()
					if( spellID ) then
...
do
	...
	function unitHasAura(unit, name, filter)
		if type(name) == "number" then
			return findAuraBySpellId(name, unit, filter)
		else
			return findAuraByName(name, unit, filter)
		end
	end
end
...
if( playerClass == "DRUID" ) then
	LoadClassData = function()
		...
		CalculateHotHealing = function(guid, spellID)
			...
			spellRank = HealComm.rankNumbers[spellRank]
...
function HealComm:UNIT_SPELLCAST_SENT(unit, spellName, spellRank, targetName)
	...
	if hotData[spellName] or spellData[spellName] then
		...
		local spellID = self.spellToID[nameID]
...
function HealComm:PLAYER_LOGIN()
	if ( not (playerClass == "DRUID" or playerClass == "PRIEST" or playerClass == "SHAMAN" or playerClass == "PALADIN") ) then
